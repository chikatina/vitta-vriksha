...
const outputDir = path.resolve(__dirname, '../store-graphics');
...
async function exportAssets() {
  ...
  const fileUrl = `file://${path.resolve('D:/Projects/ChikatiStudio/site/vitta-vriksha/store-assets.html').replace(/\\/g, '/')}`;
  ...
  if (featureBanner) {
    const box = await featureBanner.boundingBox();
    console.log('Capturing Feature Graphic...', box);
    await featureBanner.screenshot({
      path: path.join(outputDir, 'google_play_feature_graphic_1024x500.png')
    });
  }

  // 2. Capture Each Screen Card (with header & mockup) and also Phone frames alone
  ...
  await browser.close();
  console.log('All store assets successfully exported to:', outputDir);
}
